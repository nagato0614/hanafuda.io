...
- [ ] 状態取得
  - [ ] `GameService.getState()` などの API から手札／場札／獲得札を取得する。
  - [ ] フロントのストア更新ロジックをバックエンド応答に合わせて整理する。
- [ ] 山札・場札更新
...
- [ ] 選択可能カード
  - [ ] ゲームロジックから返却される選択候補リストを受け取る。
  - [ ] フロントは候補以外のカードをハイライト不可にする。
  - [ ] 逆順（場札→手札）選択にも同じロジックを適用する。
